#ifndef LISTS_H
#define LISTS_H

#include <stdio.h>
#include <stdlib.h>
#include <stddef.h> /* For size_t */

/**
 * struct dlistint_s - doubly linked list
 * @n: integer data
 * @prev: points to the previous node
 * @next: points to the next node
 *
 * Description: doubly linked list node structure for Holberton project
 */
typedef struct dlistint_s
{
    int n;
    struct dlistint_s *prev;
    struct dlistint_s *next;
} dlistint_t;

/* 0. Prints all the elements of a dlistint_t list */
size_t print_dlistint(const dlistint_t *h);

/* 1. Returns the number of elements in a dlistint_t list */
size_t dlistint_len(const dlistint_t *h);

/* 2. Adds a new node at the beginning of a dlistint_t list */
dlistint_t *add_dnodeint(dlistint_t **head, const int n);

/* 3. Adds a new node at the end of a dlistint_t list */
dlistint_t *add_dnodeint_end(dlistint_t **head, const int n);

/* 4. Frees a dlistint_t list */
void free_dlistint(dlistint_t *head);

/* 5. Returns the nth node of a dlistint_t linked list */
dlistint_t *get_dnodeint_at_index(dlistint_t *head, unsigned int index);

/* 6. Returns the sum of all the data (n) of a dlistint_t linked list */
int sum_dlistint(dlistint_t *head);

/* 7. Inserts a new node at a given position (idx) */
dlistint_t *insert_dnodeint_at_index(dlistint_t **h, unsigned int idx, int n);

/* 8. Deletes the node at index of a dlistint_t linked list */
int delete_dnodeint_at_index(dlistint_t **head, unsigned int index);

#endif /* LISTS_H */
